		...
		<view class="container" :style='{"padding":"0 36rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"100%"}'>
            <view :style='{"padding":"0","flexWrap":"wrap","background":"#FFF5EF","display":"flex","width":"100%","height":"auto","order":"2"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto","order":"6"}'>
					...
					<button :style='{"border":"0","padding":"0 40rpx","margin":"0 10rpx 10rpx","color":"#000","borderRadius":"60rpx","background":"#FF9157","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('discussshiwuzhaoling','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 40rpx","margin":"0 10rpx 10rpx","color":"#000","borderRadius":"60rpx","background":"#FF9157","width":"auto","fontSize":"28rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('discussshiwuzhaoling','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshiwuzhaolingCleanType')
			if(cleanType){
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussshiwuzhaoling', this.id);
		...
		methods: {
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussshiwuzhaoling', this.id);
				this.detail = res.data;

				if(type==2){
					this.detail.discussnum++
					await this.$api.update('discussshiwuzhaoling',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussshiwuzhaoling', {
						page: mescroll.num,
						limit: 10,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussshiwuzhaoling/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussshiwuzhaoling',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussshiwuzhaoling',that.detail)
							setTimeout(()=>{
