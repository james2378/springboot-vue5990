	...
	<view :style='{"width":"100%","padding":"0 20rpx 40rpx","position":"relative","background":"linear-gradient(180deg, #FF534C 0%, #FF9557 100%)","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#000","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.kemu" v-model="ruleForm.kemu" placeholder="科目"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`kemu`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`kemu`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`kemu`, this.ruleForm);
						}else{
							await this.$api.add(`kemu`, this.ruleForm);
						}
