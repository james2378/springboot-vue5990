	...
	<view :style='{"width":"100%","padding":"0 20rpx 40rpx","position":"relative","background":"linear-gradient(180deg, #FF534C 0%, #FF9557 100%)","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
                ...
                <xia-editor ref="editor" :style='{"minHeight":"300rpx","border":"2rpx solid #efefef","width":"100%","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxianzhishangpin`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussxianzhishangpin`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussxianzhishangpin`, this.ruleForm);
						}else{
							await this.$api.add(`discussxianzhishangpin`, this.ruleForm);
						}
						uni.setStorageSync('discussxianzhishangpinCleanType',true);
