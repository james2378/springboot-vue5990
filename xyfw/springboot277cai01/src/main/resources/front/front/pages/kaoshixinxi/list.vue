		...
		<view :style='{"padding":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"100%"}'>
			<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 72rpx)","padding":"20rpx 0","margin":"0 auto","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 0 0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #000","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"10rpx","background":"#F8F8F8","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.kaoshimingcheng" type="text" placeholder="考试名称" ></input>
				</view>
			...
			<view :style='{"width":"100%","background":"#fff","height":"auto"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"padding":"10rpx","margin":"0 auto","whiteSpace":"nowrap","background":"linear-gradient(134deg, #FF6C53 0%, #FF9257 100%)","display":"flex","width":"calc(100% - 72rpx)","height":"auto"}'>
					<view :class='categoryName === item.kemu ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.kemu)">{{item.kemu}}</view>
				</scroll-view>
					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","overflow":"hidden","position":"relative","borderRadius":"12rpx"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","flexWrap":"wrap","background":"#FF9257","display":"flex"}'>
								<view :style='{"width":"100%","padding":"0 10rpx","lineHeight":"1.5","fontSize":"28rpx","color":"#fff","order":"1"}' class="title ">{{product.kaoshimingcheng}}</view>
								<view :style='{"width":"100%","padding":"0 10rpx","lineHeight":"1.5","fontSize":"28rpx","color":"#fff","order":"1"}' class="title ">{{product.kemu}}</view>
								<view :style='{"width":"100%","padding":"0 10rpx","lineHeight":"1.5","fontSize":"28rpx","color":"#fff","order":"1"}' class="title ">{{product.kaoshiriqi}}</view>
								<view :style='{"width":"100%","padding":"0 10rpx","lineHeight":"1.5","fontSize":"28rpx","color":"#fff","order":"1"}' class="title ">{{product.kaoshishijian}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","background":"#FF9257","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshixinxi','修改')) || (!userid && isAuthFront('kaoshixinxi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshixinxi','删除')) || (!userid && isAuthFront('kaoshixinxi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","background":"#FF9257","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshixinxi','修改')) || (!userid && isAuthFront('kaoshixinxi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('kaoshixinxi','删除')) || (!userid && isAuthFront('kaoshixinxi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
								</view>
							</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('kemu', {page:1,limit:100});
            } else {
                res = await this.$api.list('kemu', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,kemu:'全部'})
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.kaoshimingcheng="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.kemu = '%' + this.categoryName + '%'
				}
				if(this.searchForm.kaoshimingcheng){
					params['kaoshimingcheng'] = '%' + this.searchForm.kaoshimingcheng + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`kaoshixinxi`, params);
                } else {
                    res = await this.$api.list(`kaoshixinxi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kaoshixinxi', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.kaoshimingcheng){
					searchForm['kaoshimingcheng'] = '%' + this.searchForm.kaoshimingcheng + '%'
				}
				if(this.categoryName!='全部'){
					searchForm.kemu = '%' + this.categoryName + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`kaoshixinxi`, searchForm);
                } else {
                    res = await this.$api.list(`kaoshixinxi`, searchForm);
                }
				// 如果是第一页数据置空
